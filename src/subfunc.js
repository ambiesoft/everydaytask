...
function getLogDate(year, month, day, timeString) {
  ...
  if (sec) ret.setSeconds(sec);
  ret.setMilliseconds(0);

  return ret;
}
function isNowBeforeStartTime(task, now) {
  if (!task.getStartTime()) return false;
...
function checkTaskTime(task) {
  try {
    if (!checkTaskTime2(task.getStartTime())) return false;
    if (!checkTaskTime2(task.getEndTime())) return false;
...
function getFaviconAsUrl(url) {
  const retUrl = `https://s2.googleusercontent.com/s2/favicons?domain_url=${encodeURIComponent(
    url
  )}`;
  return retUrl;
}
